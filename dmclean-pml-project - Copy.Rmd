...
We now proceed to the fitting of regression trees using a stochastic gradient boosting technique.
...
```
To define $K$-fold cross validation instead, we use the `trainControl` argument to `caret::train` function. In this instance, the 10-fold cross-validation is repeated `r n.repeats` times:
```{r}
...
```
Which predicts a reasonable performance in out-of-sample data given the greater majority of the probability mass (here, percentages) lie on the main diagonals.
...
As an alternative to the gradient boosted model with regression trees, we fit a neural network to the training dataset and use 10-fold cross validation to assess its out-of-sample performance. The `nnet` package will be used to fit a single hidden layer (i.e. a perceptron) via the `caret` package's `train` function as follows:
```{r}
...
if( model.Fitted.nnet ){
  ...
  # Reapply preprocessing as per saved fit
  
  # Set up sampling scheme to test fitting method by speeding it up
  # with a view to setting size=nrow(training) for full fit
  #set.seed(1)
  #my.sample    <- sample(x=1:nrow(training),size=400)
  ...
  # Pre-process the continuous neural network inputs to have a range in [0, 1]
  #~~~not working again???~~~ preProcX   <- nnetFitList$preProcessing
  ...
  # Augment the range.testing dataset with dummy factor variables
  # in order to predict with the nnet function:
  ...
  # Tune/train the network
  modFitnnet  <- train(classe ~ ., method="nnet", maxit=1000, data=range.training.dummy,
...
```
The dimension of each is roughly an equal split. For the training set: $`r dim(blending.data.train)`$; and for the testing: $`r dim(blending.data.test)`$. We now fit a random forest to the training partition.

Before fitting the blended model, we can quantify how the gradient boosted classification tree and neural net models perform relative to one-another using summaries and following cross-tabulation:
```{r}
